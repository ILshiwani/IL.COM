...
function myslider(){
    var t=$(".my-slider").cardslider({
        swipe:!0,direction:"up",afterCardChange:function(t,e){
            arroweffect(),$(`.Thumbwrap a[data-slide=${
                t+1
            }
...
function arroweffect(){
    var t=$(".cardslider-cards").find("li.cardslider-card").length,e=$(".cardslider-cards").find("li.out").length;
...
function tabs(){
    ...
    if($("#"+t).fadeIn("slow"),$(".js_tab .tab-cont").click((function(){
        ...
        var e=$(".js_tab .tab-cont.active").outerWidth(),a=$(".tab-name").outerWidth();
        let t=a/3-e/a;
        var i=0,s=0;
        let d=1;
...
function rollupcounter(){
    document.querySelectorAll(".counter").forEach((t=>{
        let e,a=[.../(\D+)?(\d+)(\D+)?(\d+)?(\D+)?/.exec(t.textContent)],i=!0;
        for(a.shift(),a=a.filter((t=>null!=t));
        t.firstChild;
        )t.removeChild(t.firstChild);
        for(let e of a)if(isNaN(e))t.insertAdjacentHTML("beforeend",`<span>${
            e
        }
        </span>`);
        else for(let a=0;
        a<e.length;
        a++)t.insertAdjacentHTML("beforeend",`<span data-value="${
            e[a]
        }
        ">\n\t\t\t\t\t\t<span>0</span>\n\t\t\t\t\t\t${
        ...
        let s=()=>{
            let a=t.getBoundingClientRect().top,d=.8*window.innerHeight;
            setTimeout((()=>{
                i=!1
            }
            ),1e3),a<d&&(setTimeout((()=>{
